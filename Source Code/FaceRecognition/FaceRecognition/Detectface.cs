...
namespace FaceRecognition
{
    ...
    {
        public static void DetectFaceEyes(
        ...
        public static void Detectface(
           ...
           List<Rectangle> faces, ParameterSetting parameter,
           out long detectionTime)
        {
            Stopwatch watch;
            ...
            using (InputArray iaImage = image.GetInputArray())
            {
                ...
                if (iaImage.Kind == InputArray.Type.CudaGpuMat && CudaInvoke.HasCuda)
                {
                    using (CudaCascadeClassifier face = new CudaCascadeClassifier(faceFileName))
                    {
                        face.ScaleFactor = parameter.ScaleFactor;//1.1
                        face.MinNeighbors = parameter.MinNeighbors;//10
                        face.MinObjectSize = Size.Empty;
                        watch = Stopwatch.StartNew();
                        using (CudaImage<Bgr, Byte> gpuImage = new CudaImage<Bgr, byte>(image))
                        using (CudaImage<Gray, Byte> gpuGray = gpuImage.Convert<Gray, Byte>())
